...
export function Remuneration({
  ...
  function calculateCommercialVehicleMarginDetail(sale: VehicleSale) {
    ...
    const warrantyPurchaseCostHT = getWarrantyPurchaseCostHT(sale);
    const marginAfterWarrantyCostHT = marginHT - warrantyPurchaseCostHT;
    const agentGrossHT = marginAfterWarrantyCostHT * 0.4;
    const responsableGrossHT = marginAfterWarrantyCostHT * 0.6;
    const vroomFeeTTC = marginTTC * VROOM_MARGIN_RATE;
  ...
  const warrantyStats = useMemo(() => {
    ...
    remunerationWarrantySales.forEach((sale) => {
      ...
      // Les garanties MEDIUM / PREMIUM / PRESTIGE ne sont plus ajoutées ici.
      // Leur prix de vente est déjà inclus dans la marge TTC de la vente.
      // On déduit donc uniquement leur prix d'achat HT dans le calcul de marge véhicule,
      // puis la marge restante est partagée normalement entre agent et responsables.
      if (soldByAgent) {
        const agentRow = baseRows.find((row) => row.type === 'agent' && Number(row.id) === Number(sale.agent_id));
    ...
    return {
      totalWarrantySales: remunerationWarrantySales.filter((sale) => {
        const key = normalizeWarrantyType(sale.warranty_type);
        return key !== 'start' && key !== 'unknown';
      }).length,
      totalWarrantyGainHT,
      unknownWarranties,
      rows: baseRows,
  ...
  const responsibleVehicleMarginStats = useMemo(() => {
    ...
    const totals = responsibleSales.reduce((acc, sale) => {
      ...
      acc.totalInstantTransferFeeHT += detail.instantTransferFeeHT;
  ...
  const commercialVehicleMarginStats = useMemo(() => {
    ...
    let totalInstantTransferFeeHT = 0;
    ...
    commercialSales.forEach((sale) => {
      ...
      totalInstantTransferFeeHT += detail.instantTransferFeeHT;
      ...
      if (agentRow) {
        ...
        agentRow.agentInstantTransferHT += detail.instantTransferFeeHT;
  ...
  function calculateWarrantyDetailForSelectedPerson(sale: VehicleSale, person: any) {
    const warrantyKey = normalizeWarrantyType(sale.warranty_type);
    const soldByAgent = saleBelongsToCommercialAgent(sale);
    const warrantyAmountTTC = Number(sale.warranty_amount || 0);
    const warrantyAmountHT = warrantyAmountTTC / TVA_DIVIDER;

    if (!sale.warranty_sold || warrantyKey === 'unknown') {
      return {
        warrantyKey,
        warrantyCountsInStats: false,
        warrantyAmountHT,
        warrantyPurchaseCostHT: 0,
        warrantyProfitHT: 0,
        selectedWarrantyShareHT: 0,
        warrantyDetailLabel: sale.warranty_sold ? 'Garantie non reconnue' : 'Aucune garantie',
      };
    }
    ...
    return {
      ...
      warrantyProfitHT,
      selectedWarrantyShareHT: 0,
      warrantyDetailLabel: `Achat garantie déduit dans la marge : -${euro(warrantyPurchaseCostHT)} HT`,
    };
  }
  ...
  const selectedVehicleDetailRows = useMemo(() => {
    ...
    return remunerationVehicleSales
      ...
      .map((sale) => {
        ...
        if (soldByAgent) {
          const detail = calculateCommercialVehicleMarginDetail(sale);
          const warrantyDetail = calculateWarrantyDetailForSelectedPerson(sale, selectedPerson);
          const marginShareHT = selectedPerson.type === 'agent' && Number(selectedPerson.id) === Number(sale.agent_id)
            ? detail.agentNetHT
            : selectedPerson.type === 'responsable'
              ? detail.responsableShareHT
              : 0;
          const selectedShareHT = marginShareHT + warrantyDetail.selectedWarrantyShareHT;
          ...
          return {
            ...
            marginShareHT,
            warrantySelectedShareHT: warrantyDetail.selectedWarrantyShareHT,
            warrantyPurchaseCostHT: warrantyDetail.warrantyPurchaseCostHT,
            warrantyCountsInStats: warrantyDetail.warrantyCountsInStats,
            warrantyDetailLabel: warrantyDetail.warrantyDetailLabel,
            selectedShareHT,
          };
        }
        ...
        const detail = calculateResponsibleVehicleMarginDetail(sale);
        const warrantyDetail = calculateWarrantyDetailForSelectedPerson(sale, selectedPerson);
        const marginShareHT = selectedPerson.type === 'responsable' ? detail.responsableShareHT : 0;
        const selectedShareHT = marginShareHT + warrantyDetail.selectedWarrantyShareHT;
        ...
        return {
          ...
          companyCashSentinelHT: detail.companyCashSentinelHT,
          agentFeesHT: detail.cashSentinelHT + detail.companyCashSentinelHT + detail.miscellaneousFeesHT + detail.instantTransferFeeHT,
          deductionsHT: (detail.vroomFeeTTC / TVA_DIVIDER) + detail.cashSentinelHT + detail.companyCashSentinelHT + detail.miscellaneousFeesHT + detail.instantTransferFeeHT,
          agentNetHT: 0,
      ...
      {showResult && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 14, alignItems: 'stretch', width: '100%' }}>
            ...
            <div className="card" style={{ borderColor: selectedProvisionalResultHT >= 0 ? '#22c55e' : '#f97316', boxShadow: '0 0 0 1px rgba(34, 197, 94, 0.18)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 14, alignItems: 'stretch', width: '100%' }}>
            ...
            <div className="card">
              <h3>Virement instantané HT</h3>
              <div className="stat-value" style={{ color: '#f97316' }}>
                -{euro(selectedPerson?.type === 'agent' ? Number(selectedCommercialMarginResult?.agentInstantTransferHT || 0) : Number(responsibleVehicleMarginStats.totalInstantTransferFeeHT || 0))}
              </div>
              <p className="muted">7,58 € HT déduit quand l'option est cochée sur une vente.</p>
            </div>
          </div>
          ...
          {selectedPerson?.type === 'agent' && (
            <div className="card" style={{ borderColor: '#22c55e' }}>
              <h3>Gain réel par véhicule</h3>
              <p className="muted">
                L'agent voit ici ce qu'il gagne réellement sur chaque véhicule : marge TTC convertie HT, 40 % agent, puis déduction des frais liés à la vente.
              </p>

              {selectedVehicleDetailRows.filter((row) => row.soldByAgent && Number(row.selectedShareHT || 0) !== 0).length === 0 ? (
                <p className="muted" style={{ marginTop: 12 }}>Aucun véhicule vendu par cet agent sur la période sélectionnée.</p>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12, marginTop: 14 }}>
                  {selectedVehicleDetailRows
                    .filter((row) => row.soldByAgent && Number(row.selectedShareHT || 0) !== 0)
                    .map((row) => (
                      <div className="item" key={`agent-real-gain-${row.id}`} style={{ display: 'grid', gap: 7 }}>
                        <div>
                          <strong>{row.vehicle}</strong>
                          <p className="muted" style={{ marginTop: 3, marginBottom: 0 }}>{row.date}{row.registration ? ` — ${row.registration}` : ''}</p>
                        </div>

                        <div style={{ display: 'grid', gap: 4, fontSize: 13 }}>
                          <span>Marge TTC : <strong>{euro(row.marginTTC)}</strong></span>
                          <span>Marge HT : <strong>{euro(row.marginHT)}</strong></span>
                          {Number(row.warrantyPurchaseCostHT || 0) > 0 && (
                            <span>Achat garantie HT : <strong>-{euro(row.warrantyPurchaseCostHT)}</strong></span>
                          )}
                          ...
                          {Number(row.warrantySelectedShareHT || 0) !== 0 && (
                            <span>
                              {row.warrantyDetailLabel} : <strong style={{ color: row.warrantySelectedShareHT >= 0 ? '#22c55e' : '#f97316' }}>
                        ...
                        <div style={{ borderTop: '1px solid rgba(148, 163, 184, 0.22)', paddingTop: 8 }}>
                          <span className="muted">Gain réel agent</span>
